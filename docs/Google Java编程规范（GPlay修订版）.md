...
#### 3.3.1 没有用到的类不要import

#### 3.3.2 尽可能避免使用静态导入（import static）
...
1.  所有的静态导入独立成组
2.  `android` imports
2.  `com.skydragon` imports(仅当这个源文件是在`com.skydragon`包下)
3.  第三方的包。每个顶级包为一组，字典序。例如：com, junit, org, sun
4.  `java` imports
5.  `javax` imports
...
一个空的块状结构里什么也不包含，大括号可以简洁地写成<font color=red>`{}`</font>，不需要换行。例外：如果它是一个多块语句的一部分(if/else 或 try/catch/finally)
 ，即使大括号内没内容，右大括号也要换行。
...
一个项目可以选择一行100个字符的列限制，除了下述例外，任何一行如果超过这个字符数限制，应该考虑换行。（<font color=red>Android Studio设置:Preferences-->Editor-->Code Style--> Right margin(columns)</font>）
...
**术语说明**：一般情况下，一行长代码为了避免超出列限制(100个字符)而被分为多行，我们称之为自动换行(line-wrapping)。
...
1.  如果在<font color=red>`非赋值运算符`</font>处断开，那么在该符号前断开(比如+，它将位于下一行)。注意：这一点与Google其它语言的编程风格不同(如C++和JavaScript)。
    这条规则也适用于以下“类运算符”符号：点分隔符(.)，类型界限中的&（<font color=red>`<T extends Foo & Bar>`</font>)，catch块中的管道符号(<font color=red>`catch (FooException | BarException e`</font>)
2.  如果在<font color=red>`赋值运算符`</font>处断开，通常的做法是在该符号后断开(比如=，它与前面的内容留在同一行)。这条规则也适用于<font color=red>`foreach`</font>语句中的分号。
...
3.  在任何左大括号前(`<span class="p">{</span>`)，两个例外：  
...
>> Android Studio配置：Editor > Code Style > Java > Spaces
...
这些名字通常是名词或名词短语。一个类的成员非常量字段统一以`m`作为前缀，静态成员非常量统一以`s`作为前缀。
